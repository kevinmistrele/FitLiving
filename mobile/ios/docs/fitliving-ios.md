# Especificação Funcional — Versão iOS
...
App pessoal de uso único para iPhone, espelhando a versão web e compartilhando a mesma conta e os mesmos dados na nuvem. O que muda é a experiência: adaptada ao toque e aos recursos do celular.
...
- Dados sincronizados com a versão web (mesma conta).
...
- Login com método simples (conta Google ou email/senha), preferindo recursos nativos de autenticação do iPhone quando possível.
...
Mesmo conteúdo da web, com experiência adaptada ao iPhone.
...
- Registrar por semana: data (seletor de data nativo), peso (kg, teclado numérico), cintura (cm) e observações.
...
- Possibilidade de **widget** mostrando peso atual e progresso rumo à meta.

> **Sobre criar app iOS sem Mac:** vale confirmar. Publicar na App Store geralmente exige conta de desenvolvedor Apple e, na prática, um Mac para o build final — embora existam serviços de build em nuvem que contornam parte disso. Para uso pessoal, a versão web instalada na tela inicial do iPhone entrega quase tudo de um app nativo, sem Mac e sem App Store. Confirme essa etapa antes de investir na versão nativa.
...
- Todos vinculados à conta e sincronizados na nuvem (mesmos dados da web).
...
4. Módulo Dieta e Metas.
5. Extras do iOS: notificações e widget.