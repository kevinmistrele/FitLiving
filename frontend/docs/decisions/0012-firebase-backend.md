...
- Deploying `firestore.rules` (`firebase deploy --only firestore:rules`) and enabling Email/Password sign-in + creating the owner's account are manual steps in the Firebase console/CLI, outside this change's scope. So is disabling public sign-up (Firebase Console → Authentication → Settings → "Prevent account creation") — the app having no sign-up UI is a product choice, not a security boundary, since the public `apiKey` still lets anyone call the Identity Toolkit REST API directly (see [security.md](../standards/security.md#auth)).
